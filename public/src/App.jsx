...
export default function CarnetNotes() {
  ...
  const buildWorkbook = () => {
    ...
    ranking.forEach((r) => {
      const moyenneCell = r.moyenne !== null ? Math.round(r.moyenne * 100) / 100 : r.absent ? "Absent(e)" : "-";
      const decision = r.absent ? "Absent(e)" : r.moyenne === null ? "-" : r.moyenne >= PASSING_AVERAGE ? "Admis(e)" : "Non admis(e)";
      recapData.push([r.nom, r.prenoms, r.matricule, moyenneCell, r.rang ?? "-", decision]);
    });
...
function RecapTab({ ranking }) {
  ...
  const classAverage =
    withMoy.length > 0 ? withMoy.reduce((acc, r) => acc + r.moyenne, 0) / withMoy.length : null;
  const admisCount = withMoy.filter((r) => r.moyenne >= PASSING_AVERAGE).length;

  return (
    <div className="rounded-xl overflow-hidden" style={{ background: T.card, border: `1px solid ${T.line}` }}>
      <div className="px-4 py-3" style={{ background: T.ink, color: "#FFFFFF" }}>
        <span style={{ fontFamily: "'Fraunces', serif" }} className="font-semibold text-lg block mb-1">
          Classement général
        </span>
        <span className="text-sm" style={{ color: T.goldSoft, fontFamily: "'IBM Plex Mono', monospace" }}>
          {admisCount} admis sur {ranking.length} · seuil : {PASSING_AVERAGE}/20
          {classAverage !== null ? ` · moyenne classe : ${formatNum(classAverage)}/20` : ""}
        </span>
      </div>
      <table className="w-full text-base">
        <thead>
          <tr style={{ background: T.goldSoft }}>
            <th className="text-left px-2 py-2 font-medium" style={{ color: T.inkSoft }}>Rang</th>
            <th className="text-left px-2 py-2 font-medium" style={{ color: T.inkSoft }}>Élève</th>
            <th className="text-right px-2 py-2 font-medium" style={{ color: T.inkSoft }}>Moy.</th>
            <th className="text-center px-2 py-2 font-medium" style={{ color: T.inkSoft }}>Décision</th>
          </tr>
        </thead>
        <tbody>
          {ranking.map((r, i) => {
                ...
                <td className="px-2 py-2 text-center">
                  {r.moyenne === null ? (
                    ...
                    <span
                      ...
                      style={{ background: isPassing ? T.greenSoft : T.redSoft, color: isPassing ? T.green : T.red }}
                    >
